...
const toDateSafe = (value: unknown): Date | null => {
  if (!value) return null; if (value instanceof Date) return value; if (typeof (value as { toDate?: () => Date })?.toDate === "function") {
    try {
...
const resolveArea = (order: FirebaseFirestore.DocumentData): string => {
  const directArea = readString(order.area || order.section || order.floorArea); if (directArea) return directArea; const tableNo = resolveTableNo(order).toUpperCase(); if (tableNo.startsWith("OD")) return "Outdoor"; if (tableNo) return "Indoor"; return "";
};
...
const resolveCustomerField = (order: FirebaseFirestore.DocumentData, key: string): string => {
  const customer = order.customer || {}; return readString(order[key]) || readString(customer[key]) || "NA";
};
...
const resolveVariation = (item: FirebaseFirestore.DocumentData): string => {
  const directVariation = readString(item.variation); if (directVariation) return directVariation; if (Array.isArray(item.variations) && item.variations.length > 0) {
    const first = item.variations[0] || {}; return readString(first.name) || readString(first.option) || readString(first.type) || readString(first.value);
  } return "";
};
const distributeAmount = (target: number, base: number, total: number): number => {
  if (!Number.isFinite(target) || !Number.isFinite(base) || !Number.isFinite(total) || base <= 0 || total <= 0) return 0; return Math.round((target * base / total) * 100) / 100;
};
const fetchDocById = async (collectionName: string, id: string): Promise<FirebaseFirestore.DocumentData | null> => {
  const resolvedId = readString(id); if (!resolvedId) return null; const snapshot = await db.collection(collectionName).doc(resolvedId).get(); return snapshot.exists ? snapshot.data() || null : null;
};
...
const getItemInvoiceDetailsReportData = async (filters: ReportFilters): Promise<ReportResponse> => {
  ...
  const ordersQuery: FirebaseFirestore.Query<FirebaseFirestore.DocumentData> =
