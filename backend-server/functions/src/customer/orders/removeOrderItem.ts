...
const getItemTotal = (item: any): number => {
  const explicitTotal = Number(item?.totalPrice); if (Number.isFinite(explicitTotal) && explicitTotal > 0) return explicitTotal; const qty = Math.max(1, Number(item?.qty ?? item?.quantity ?? 1)); const price = Number(item?.finalUnitPrice ?? item?.price ?? item?.priceRaw ?? 0); return Number.isFinite(price) ? qty * price : 0;
};
...
export const removeOrderItem = functions.https.onRequest(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!outletId || !orderId || !itemId) {
      res.status(400).json({success: false, message: "Missing required fields: outletId, orderId, and itemId"}); return;
    }
    ...
    const transactionResult = await db.runTransaction(async (tx) => {
      ...
      if (items.length <= 1) {
        ...
        if (sessionId) {
          const sessionOrdersSnap = await tx.get(db.collection("outlets").doc(outletId).collection("orders").where("sessionId", "==", sessionId).limit(50)); sessionOrdersSnap.docs.forEach((doc) => relatedOrderDocs.set(doc.id, doc));
        }
        if (tableId) {
          const tableOrdersSnap = await tx.get(db.collection("outlets").doc(outletId).collection("orders").where("tableId", "==", tableId).limit(50)); tableOrdersSnap.docs.forEach((doc) => relatedOrderDocs.set(doc.id, doc));
        }
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "OUTLET_MISMATCH") {
        res.status(400).json({success: false, message: "Order does not belong to this outlet"}); return;
      }
      if (error.message === "ORDER_NOT_ACTIVE") {
        res.status(409).json({success: false, message: "Order is not active"}); return;
      }
      if (error.message === "ITEM_NOT_FOUND") {
        res.status(404).json({success: false, message: "Item not found in order"}); return;
      }
      if (error.message === "CANNOT_REMOVE_LAST_ITEM") {
        res.status(400).json({success: false, message: "Cannot remove the final item. Please cancel the entire order instead."}); return;
      }
    }
