...
const isActiveOrder = (orderData: FirebaseFirestore.DocumentData): boolean => {
  const status = String(orderData.status || orderData.orderStatus || orderData.orderLifecycleStatus || "")
    .trim()
    .toUpperCase();
  return !["CANCELLED", "CLOSED", "DELETED"].includes(status);
};
...
const getItemTotal = (item: any): number => {
  ...
  const price = Number(item?.finalUnitPrice ?? item?.price ?? item?.priceRaw ?? 0);
  return Number.isFinite(price) ? qty * price : 0;
};

const getOrderItems = (orderData: FirebaseFirestore.DocumentData): any[] => {
  return Array.isArray(orderData.items) ? orderData.items : [];
};

export const removeOrderItem = functions.https.onRequest(
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const { outletId, orderId, itemId } = req.body as {
        outletId?: string;
        orderId?: string;
        itemId?: string;
      };
      ...
      if (!outletId || !orderId || !itemId) {
        res.status(400).json({
          success: false,
          message: "Missing required fields: outletId, orderId, and itemId",
        });
        return;
      }
      ...
      const transactionResult = await db.runTransaction(async (tx) => {
        ...
        if (items.length <= 1) {
          ...
          if (sessionId) {
            const sessionOrdersSnap = await tx.get(
              db.collection("orders")
                .where("sessionId", "==", sessionId)
                .where("outletId", "==", outletId)
                .limit(50)
            );
            sessionOrdersSnap.docs.forEach((doc) => relatedOrderDocs.set(doc.id, doc));
          }

          if (tableId) {
            const tableOrdersSnap = await tx.get(
              db.collection("orders")
                .where("tableId", "==", tableId)
                .where("outletId", "==", outletId)
                .limit(50)
            );
            tableOrdersSnap.docs.forEach((doc) => relatedOrderDocs.set(doc.id, doc));
          }

          if (relatedOrderDocs.size === 0) {
            relatedOrderDocs.set(orderSnap.id, orderSnap as FirebaseFirestore.QueryDocumentSnapshot);
          }

          const activeTableItemCount = Array.from(relatedOrderDocs.values()).reduce((count, doc) => {
            const data = doc.data();
            return isActiveOrder(data) ? count + getOrderItems(data).length : count;
          }, 0);

          if (activeTableItemCount <= 1) {
        ...
        // Recalculate discounts (applied offers)
        ...
        return {
          ...
          ...orderData,
          ...updatePayload,
          updatedAt: new Date().toISOString(), // Serialized timestamp for response
        };
      });
      ...
      if (error instanceof Error) {
        ...
        if (error.message === "OUTLET_MISMATCH") {
          res.status(400).json({ success: false, message: "Order does not belong to this outlet" });
          return;
        }
        ...
        if (error.message === "CANNOT_REMOVE_LAST_ITEM") {
          res.status(400).json({
            success: false,
            message: "Cannot remove the final item. Please cancel the entire order instead.",
          });
          return;
        }
      }
