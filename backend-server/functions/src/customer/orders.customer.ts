...
// const normalizePersistedOrderItem = (item: PersistedOrderItem): PersistedOrderItem => {
//   const qty = sanitizeQty((item as any).qty ?? (item as any).quantity) || 1;
//   const finalUnitPrice = readNumber(item.finalUnitPrice ?? (item as any).price ?? item.basePrice, 0);
//   return {
//     ...item,
//     id: String((item as any).id || item.productId || ""),
//     productId: String(item.productId || (item as any).id || ""),
//     qty,
//     quantity: qty,
//     status: (item as any).status || "in-progress",
//     price: finalUnitPrice,
//     finalUnitPrice,
//     totalPrice: readNumber(item.totalPrice, finalUnitPrice * qty),
//   };
// };
...
//         const mergedItems = existingItems.map(normalizePersistedOrderItem).concat(newItems);
